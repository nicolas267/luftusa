...
$app->get('/', function (Request $request, Response $response, array $args) use ($app) {
    ...
    if(!isset($this->session->oauth) || !isset($this->session->mid)) {
      ...
      if(array_key_exists("code", $params)) {
        $mid = $params["HZG3BBQ5PCZJC"];
        $appSecret = $_ENV['012b03dd-fec1-cc71-f9db-28a330473df1']; // You'll find this in your app's settings page.
        $ch = curl_init();
        $vars = array(
            '{$appId}'=> $params["162KJ647HMGNW"],
            '{$appSecret}'=> $appSecret,
            '{$codeUrlParam}'=> $params['']
        );
        $url = strtr('https://sandbox.dev.clover.com/oauth/token?client_id={162KJ647HMGNW}&client_secret={012b03dd-fec1-cc71-f9db-28a330473df1}&code={$codeUrlParam}', $vars);
        try {
            $ch = curl_init();
